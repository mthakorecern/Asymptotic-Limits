    ...
    for channel in channels:
        ...
        for year in years:
            if channel == "inclusive":
                original_path = (
                    f"{year}_{postfix}/combined_cards_{year}/combined_{year}_{mass}.txt"
                )
            else:
        ...
        try:
            ...
            workspace_file = os.path.join(
                output_dir, f"combined_2016_{channel}_{mass}.root"
            )
            ...
            combine_result_file = os.path.join(
                output_dir, f"combine_result_2016_{channel}_{mass}.log"
            )
            ...
            shutil.move(
                combined_file,
                os.path.join(combined_cards_dir, f"combined_2016_{channel}_{mass}.txt"),
            )
            shutil.move(
                workspace_file,
                os.path.join(workspaces_dir, f"combined_2016_{channel}_{mass}.root"),
            )
            subprocess.run(
                f"mv higgsCombineTest.AsymptoticLimits.mH*.root {channel_output_dir}/.",
                shell=True,
                check=True,
            )
            shutil.move(
                combine_result_file,
                os.path.join(logs_dir, f"combine_result_2016_{channel}_{mass}.log"),
            )

        except subprocess.CalledProcessError as e:
            print(
                f"Error during processing for mass point {mass}, channel {channel}: {e}"
            )
...
def plot_limits(json_file, output_dir, channel):
    ...
    if channel == "tautau":
        ...
        plt.figtext(
            0.14, 0.82, "Inclusive", ha="left", va="top", fontsize=12, fontweight="bold"
        )

    plt.figtext(
        0.88, 0.92, "36.67 $fb^{-1}$, 13 TeV", ha="right", va="top", fontsize=12
    )
