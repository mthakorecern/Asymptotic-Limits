...
for mass in mass_points:
    ...
    try:
        ...
        combine_command = (
            f"combine -M AsymptoticLimits {os.path.basename(workspace_file)} -m {mass}"
        )
        print(f"\n\n")
        ...
        shutil.move(
            combined_run2_file,
            os.path.join(combined_cards_dir, f"combined_run2_{mass}.txt"),
        )
        shutil.move(
            workspace_file, os.path.join(workspaces_dir, f"combined_run2_{mass}.root")
        )
        # Move the combine output files to the appropriate location
        subprocess.run(
            f"mv higgsCombineTest.AsymptoticLimits.mH*.root {combine_output_dir}/.",
            shell=True,
            check=True,
        )
...
def plot_limits(json_file, output_dir):
    ...
    plt.figtext(
        0.88, 0.92, "138.12 $fb^{-1}$, 13 TeV", ha="right", va="top", fontsize=12
    )
