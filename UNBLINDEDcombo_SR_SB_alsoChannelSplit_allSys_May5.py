...
def plot_limits(json_file, output_dir, year, channel=None):
    ...
    plt.figtext(0.12, 0.92, "CMS Preliminary", ha="left", va="top", fontsize=12, fontweight='bold')
    plt.figtext(0.88, 0.92, f"({year}, 13 TeV)", ha="right", va="top", fontsize=12)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run Combine Harvester with custom histogram path, year, and parent directory")
    parser.add_argument('--histogram_path', type=str, required=True, help="Path to the ROOT file containing histograms")
    parser.add_argument('--year', type=str, required=True, help="Year of the dataset (e.g., 2018)")
    parser.add_argument('--parent_dir', type=str, required=True, help="Parent directory for storing outputs")
    ...
    for bin in range(1, n_bins + 1):
        ...
        mass_bins[f"{lower_edge}_{upper_edge}"] = f"{year}_allHists_SignalRegion_1fb_varbin_mass_{lower_edge}_{upper_edge}.root"
    ...
    for bin in range(1, n_bins + 1):
        lower_edge = str(bin_edges[bin - 1])
        upper_edge = str(bin_edges[bin])
        mass_bins_sideband[f"{lower_edge}_{upper_edge}"] = f"{year}_allHists_SideBand_1fb_varbin_mass_{lower_edge}_{upper_edge}.root"
    ...
    signal_masses = ['1000', '1200', '1400', '1600', '1800', '2000', '2500', '3000', '3500', '4000', '4500']
    for signal_mass in signal_masses:
        ...
        for mass_range, root_file in mass_bins_sideband.items():
            ...
            cats = [(1, f'tautau_{year}_{mass_range}_SB'), 
            ...
            lnN_systematics = [
                ("lumi%s"%(year), "lnN", {"others": lumi_uncorr_uncertainties[year],"radion": lumi_uncorr_uncertainties[year]}),
                ("lumi_1718", "lnN", {"others": lumi_partial_uncertainties[year],"radion":lumi_partial_uncertainties[year]}) if args.year == "2017" or args.year == "2018" else None,  # Luminosity for 2017-2018
                ("lumi_161718", "lnN", {"others": lumi_corr_uncertainties[year],"radion": lumi_corr_uncertainties[year]}),
                ...
                ("jesBBEC1_%s"%(jes_year_unc[year]), "lnN",{"others": 1.02,"radion":1.02}),
                ("jesEC2", "lnN",{"others": 1.02,"radion":1.02}),
                ("jesEC2_%s"%(jes_year_unc[year]), "lnN",{"others": 1.02,"radion":1.02}),
                ("jesFlavorQCD", "lnN",{"others": 1.02,"radion":1.02}),
                ("jesHF", "lnN",{"others": 1.02,"radion":1.02}),
                ("jesHF_%s"%(jes_year_unc[year]), "lnN",{"others": 1.02,"radion":1.02}),
                ("jesRelativeBal", "lnN",{"others": 1.05,"radion":1.05}),
                ("jesRelativeSample_%s"%(jes_year_unc[year]), "lnN",{"others": 1.05}),
                #("L1PreFiringWeight_Nom", "lnN",{"others": 1.01,"radion":1.01}),
                ("L1PreFiringWeight", "lnN",{"others": l1prefiring_uncertainties[year],"radion":l1prefiring_uncertainties[year]}),
                (f'boostedDeepTauid', "lnN", {
                    f'leptau_{year}_{mass_range}_SB': {"others": 1.10, "radion": 1.10},
                    f'tautau_{year}_{mass_range}_SB': {"others": 1.20, "radion": 1.20}  # Ensure this is handled gracefully
                }),
                #The above needs to be updated later
                ("metsfWeight", "lnN", {"others": 1.012, "radion": 1.012}),  # Fixed format
                ("hbblooseWeight", "lnN", {"radion": 1.05}),  # Signal-specific weight
                ("eleidWeight", "lnN", {
                    f'leptau_{year}_{mass_range}_SB': {"others": 1.004, "radion": 1.004},
                ...
                ("muonidWeight", "lnN", {
                    f'leptau_{year}_{mass_range}_SB': {"others": 1.01, "radion": 1.01},
                    f'tautau_{year}_{mass_range}_SB': {}
                }),
                ("muonisoWeight", "lnN", {
                    f'leptau_{year}_{mass_range}_SB': {"others": 1.001, "radion": 1.001},
                    f'tautau_{year}_{mass_range}_SB': {}
                }),
            ]
            ...
            shape_systematics = [
                ...
                ("jesRelativeSample_%s"%(jes_year_unc[year]), "shape", {"*": ["radion"]}),
                ...
                ("btagmediumWeightlight%s"%(year), "shape", {"*": ["others", "radion"]}),
                #Theory
            ...
            # Add log-normal systematics
            for sys_name, sys_type, sys_vals in lnN_systematics:
                if sys_vals is None:  # Skip None values (e.g., lumi_1718 when year is not 2017 or 2018)
                    continue
            ...
            for sys_name, sys_type, sys_categories in shape_systematics:
                for cat, procs in sys_categories.items():
                    cb.cp().bin([cat] if cat != "*" else [f'tautau_{year}_{mass_range}_SB', f'leptau_{year}_{mass_range}_SB']).process(procs).AddSyst(
                        cb, sys_name, sys_type, ch.SystMap()(1.0)
                    )
            ...
            output = ROOT.TFile(f"{datacard_dir}/YEAR_{year}_RANGE_{mass_range}_SideBand_MASSPOINT_{signal_mass}_1fb_varbin.root", "RECREATE")
            ...
            for b in cb.bin_set():
                ...
                if mass_range in b:
                    ...
                    for proc in cb.cp().bin([b]).backgrounds().process_set():
                        ...
                        print ("For the process..",proc,"...in the bin...","...rate..",rate)
                        total_yield += rate
                    if total_yield == 0:
                        # Set a small yield value for each process in the empty bin
                        for proc in cb.cp().bin([b]).backgrounds().process_set():
                            print("Filling the small value for...",proc)
                            cb.cp().bin([b]).process([proc]).ForEachProc(lambda p: p.set_rate(small_value))
                            print("After filling the rate = ",cb.cp().bin([b]).process([proc]).GetRate())
                    ...
                    for m in masses:
                        ...
                        with open(datacard_filename, 'a') as out:
                            ...
                            out.write(f"rate_top_{find_channel(b)}_{year}_{mass_range} rateParam {b} top 1.0 [0.01,10]\n")
    ...
    for signal_mass in signal_masses:
        ...
        for mass_range, root_file in mass_bins.items():
            ...
            cats = [(1, f'tautau_{year}_{mass_range}_SR'), 
            ...
            lnN_systematics = [
                ...
                ("L1PreFiringWeight", "lnN",{"others": l1prefiring_uncertainties[year],"radion":l1prefiring_uncertainties[year]}),
                (f'boostedDeepTauid', "lnN", {
                    f'leptau_{year}_{mass_range}_SR': {"others": 1.10, "radion": 1.10},
                    f'tautau_{year}_{mass_range}_SR': {"others": 1.20, "radion": 1.20}  # Ensure this is handled gracefully
                }),
                ("metsfWeight", "lnN", {"others": 1.012, "radion": 1.012}),  # Fixed format
                ("hbblooseWeight", "lnN", {"radion": 1.05}),  # Signal-specific weight
                ("eleidWeight", "lnN", {
                    f'leptau_{year}_{mass_range}_SR': {"others": 1.004, "radion": 1.004},
                ...
                ("muonidWeight", "lnN", {
                    f'leptau_{year}_{mass_range}_SR': {"others": 1.01, "radion": 1.01},
                    f'tautau_{year}_{mass_range}_SR': {}
                }),
                ("muonisoWeight", "lnN", {
                    f'leptau_{year}_{mass_range}_SR': {"others": 1.001, "radion": 1.001},
                    f'tautau_{year}_{mass_range}_SR': {}
                }),
            ]
            ...
            for sys_name, sys_type, sys_categories in shape_systematics:
                for cat, procs in sys_categories.items():
                    cb.cp().bin([cat] if cat != "*" else [f'tautau_{year}_{mass_range}_SR', f'leptau_{year}_{mass_range}_SR']).process(procs).AddSyst(
                        cb, sys_name, sys_type, ch.SystMap()(1.0)
                    )
            ...
            # Create an output root file for the current mass range
            output = ROOT.TFile(f"{datacard_dir}/YEAR_{year}_RANGE_{mass_range}_SignalRegion_MASSPOINT_{signal_mass}_1fb_varbin.root", "RECREATE")
    ...
    for m in signal_masses:
       for channel in channels:
           ...
           combined_card = f"{combined_dir}/{channel}/combined_{channel}_{year}_{m}.txt"
           os.makedirs(f"{combined_dir}/{channel}", exist_ok=True)  # Ensure directory exists
           ...
           run_command(f"mv higgsCombineTest.AsymptoticLimits.mH{m}.root {output_dir_channel}/")
    ...
    for m in signal_masses:
        ...
        command_full = f"combineCards.py {' '.join(channel_commands)} > {combined_card_full}"
        ...
        print(f"Running T2W command: ", f"text2workspace.py {combined_card_full} -o {workspace_file} -m {m}")
        run_command(f"text2workspace.py {combined_card_full} -o {workspace_file} -m {m}")
    ...
    run_command(f"combineTool.py -M CollectLimits {output_dir}/higgsCombineTest.AsymptoticLimits.mH*.root -o {limit_json}")
    ...
    for channel in ['leptau','tautau']:
        ...
        run_command(f"combineTool.py -M CollectLimits {output_dir}/{channel}/higgsCombineTest.AsymptoticLimits.mH*.root -o {limit_json_channel}")
